...
`model/load.js` is the only place the SPA fetches a graph, a registry or a scenario index. Every
component reads through `loadGraph()`, so changing data source is never a component change. The mode
comes from `VITE_FGM_SOURCE`.

This module fetches. `apps.js` and `fgm.js` validate.

## App discovery

Apps are **discovered, not registered**. Each app owns a `fact-explorer.app.json` at its repo root,
