...
//
// Kicked off at import so the fetch is in flight before the element upgrades. The nav ships in
// production and is the first thing on the page, so hosts should also preload it, and
...
// Swallowed here. loadTemplates() drops its memo on failure, so the awaited call below refetches
// and surfaces the error to the element that actually needs the markup.
