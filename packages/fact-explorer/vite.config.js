...
// There is deliberately no `optimizeDeps: { include: … }` for taxpert, and this note is here so one
// is not added. Vite treats a linked (`file:`) dependency as source and does not pre-bundle it,
// which costs ~50 dev-server requests, but the package cannot be handed to the dep optimizer at
...
//
// The waterfall is still worth removing, and the shape that can work is the pre-built bundle: it
// inlines its templates and calls registerTemplates(), so nothing depends on import.meta.url and
// there is one copy of everything. It names its shared-seam modules as `../../shared/js/*.js`,
// which resolves in the tree `make copy-shared-ui` lays out rather than in this one, so consuming
// it here needs that seam resolved for this tree first. See
// ../../docs/internals/bundled-build.md.

// Serve public/data/ ourselves, compressed.
//
// The Fact Explorer people actually open is this dev server rather than the nginx in its image,
// because the compose override builds `target: build` and runs `npm run dev`. Vite's dev server
// does not compress and has no configuration that makes it, so this is that middleware.
//
// Caching is deliberately revalidation rather than a max-age. These files are regenerated by
// `npm run make-fgm` in the middle of a working session, and a real max-age would serve
// yesterday's graph until someone thought to hard-refresh. A strong ETag makes a repeat load a 304
// with no body. See ../../docs/internals/fact-explorer-internals.md.
//
