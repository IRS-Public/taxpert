...
/**
 * Every directory scanned for apps: the one above, then anything in FORM_BUILDER_EXTRA_APPS_DIRS
 * (colon-separated, in the shell PATH style).
 *
 * The extra roots exist for the container. `apps/` is a single directory precisely so it can be one
 * bind mount, and it is mounted read-only — which means a *second* mount cannot be nested inside it:
 * Docker has to create the mountpoint in the parent, and the parent is read-only. (Making it
 * writable is worse than it sounds; the mkdir would land in the host's apps directory.) So an
 * application that lives outside the apps directory — one generated by cookiecutter wherever the
 * author happened to be standing — is mounted at its own root instead, and named here.
 *
 * First root wins on a duplicate id, so the apps directory stays authoritative.
 */
export function appsDirs() {
  const extra = (process.env.FORM_BUILDER_EXTRA_APPS_DIRS ?? '')
    .split(':')
    .filter(Boolean)
    .map((dir) => resolve(dir))
  return [...new Set([appsDir(), ...extra])]
}

/**
 * Every directory under the given root, or roots, that carries a descriptor, in directory order.
...
export function discoverDescriptors(scanRoot) {
  const seen = new Set()
  return [scanRoot].flat().flatMap((root) => {
    if (!existsSync(root)) return []
    return readdirSync(root, { withFileTypes: true })
      .filter(
        (d) => (d.isDirectory() || d.isSymbolicLink()) && existsSync(join(root, d.name, DESCRIPTOR))
      )
      .map((d) => ({ repoDir: join(root, d.name), ...readJson(join(root, d.name, DESCRIPTOR)) }))
...
export function buildRegistry(scanRoot) {
  const roots = [scanRoot].flat()
  const descriptors = discoverDescriptors(roots)
  if (!descriptors.length) {
    throw new Error(
      `No ${DESCRIPTOR} found in ${roots.join(', ')}.\n` +
  ...
  // Ordering and the default app are the apps directory's to state, so the first root that carries
  // an overrides file wins. An app mounted from outside it does not get to reorder the menu.
  const overridesPath = roots.map((root) => join(root, OVERRIDES)).find((path) => existsSync(path))
  const overrides = overridesPath ? readJson(overridesPath) : null
...
function main(argv) {
  ...
  // --root names one directory and means only that one; without it, every root appsDirs() knows.
  const rootFlag = argv.indexOf('--root')
  const scanRoot = rootFlag === -1 ? appsDirs() : resolve(argv[rootFlag + 1])
